...
- `GET /api/v1/admin/users` — 管理员用户列表（支持 keyword/user_type/is_blacklisted 筛选，分页）
- `GET /api/v1/admin/users/:id` — 管理员查看用户详情（手机号明文）
...
- **禁止未经明确允许的 `git push`**。任何推送操作（包括 `git push`、`git push --force` 等）必须事先获得用户许可，否则不得执行。本地提交可以自由进行，但推送到远程需用户确认。
